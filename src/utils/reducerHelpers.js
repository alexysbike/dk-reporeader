...
export const spliceReducer = (...keys) => (state, index) =>
  update(state, keys.reduceRight(keyReduceFunction('$splice', [[index, 1]]), {}));

export const regularSetByShaReducer = (...keys) => (state, item) =>
  update(state, keys.reduceRight(keyReduceFunction(item.sha, { $set: item }), {}));
